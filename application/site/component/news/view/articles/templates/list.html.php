...
<? foreach ($articles as $article) : ?>
    <div class="media<?= !$article->attachments_attachment_id ? ' media--imageless' : ''; ?>">
        <? if($article->attachments_attachment_id): ?>
            <a tabindex="-1" class="pull-left thumbnail media-object" href="<?= '/'.$site.'/'.translate('news').'/'.$article->id.'-'.$article->slug ?>">
        ...
        <div class="media-body">
            <a class="media-heading" href="<?= '/'.$site.'/'.translate('news').'/'.$article->id.'-'.$article->slug ?>"><?= $article->title ?></a>
